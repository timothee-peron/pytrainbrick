...
class Peripheral:
    ...
    def _convert_bytes(self, msg_bytes: bytearray, byte_count):
        ...
        if byte_count == 1:  # just a uint8
            ...
            logging.error(f'Cannot convert array of {msg_bytes} length {len(msg_bytes)} to python datatype')
            val = None
        return val
    ...
    async def _parse_combined_sensor_values(self, msg: bytearray):
        ...
        """
        msg.pop(
            0)  # Remove the leading 0 (since we never have more than 7 datasets even with all the combo modes activated
