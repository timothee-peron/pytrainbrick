...
class Motor(Peripheral):
    ...
    async def _cancel_existing_differet_ramp(self):
        ...
        if self.ramp_in_progress_task:
            ...
            if current != self.ramp_in_progress_task:
                ...
                self.ramp_in_progress_task = None
                logging.debug(f'Canceling previous speed ramp in progress')
    ...
    async def ramp_speed(self, target_speed, ramp_time_ms):
        ...
        logging.debug(f'ramp_speed steps: {number_of_steps}, speed_diff: {speed_diff}, speed_step: {speed_step}')
        current_step = 0
        ...
        async def _ramp_speed():
            ...
            while current_step < number_of_steps:
                ...
                logging.debug(f'Setting next_speed: {next_speed}')
                current_step += 1
        ...
        logging.debug(f'Starting ramp of speed: {start_speed} -> {target_speed} ({ramp_time_ms / 1000}s)')
